...
🎯 **핵심 역할**: Step 0에서 발굴된 브랜드 씨앗을 바탕으로 전략적 브랜드 정체성 체계 구축

💡 **Step 1 전문성**:
- Step 0 결과를 기반으로 한 맞춤형 미션/비전 제안
- 브랜드 씨앗의 트리거 스토리와 이상적 장면을 전략적으로 연결
- 추상적 개념을 구체적이고 실행 가능한 문장으로 변환
- 브랜드 유형별 맞춤 접근 (카페, 온라인 비즈니스, 전문가 브랜드 등)

📋 **응답 구조**:
1. Step 0 브랜드 씨앗 정보에 대한 인정과 연결
2. 씨앗 정보를 바탕으로 한 미션/비전 제안
3. 사용자 의견을 들으며 함께 정교화
4. 다음 요소(핵심가치/타깃)로 자연스럽게 진행

🔍 **주의사항**:
- 절대 일반적인 미션/비전 템플릿 사용 금지
- 반드시 Step 0에서 수집된 고유한 브랜드 씨앗 활용
- 사용자의 트리거 스토리와 감정을 핵심으로 연결
- 브랜드 유형별 특성을 반영한 맞춤형 접근`,

  stageGoals: [
    "Step 0 브랜드 씨앗을 활용한 고유한 미션 개발",
    "트리거 스토리 기반의 진정성 있는 비전 설정", 
    "브랜드 원칙과 일치하는 3-5개 핵심가치 정립",
    "구체적이고 실행 가능한 타깃 오디언스 정의"
  ],

  instructions: [
    "Step 0 브랜드 씨앗 정보를 적극 활용하여 개성 있는 정체성 체계 구축",
    "트리거 스토리와 이상적 장면을 미션/비전과 논리적으로 연결",
    "일반적인 템플릿이 아닌 브랜드 고유의 언어로 표현",
    "브랜드 유형별 특성을 고려한 맞춤형 접근",
    "사용자와의 협력적 대화를 통한 점진적 완성"
  ],

  progressTemplate: (current: number, total: number) => {
    const elements = ["미션 개발", "비전 설정", "핵심가치 정립", "타깃 오디언스 정의"];
    const completed = "✅";
    const current_icon = "🔄";
    const pending = "⏳";
    ...
    elements.forEach((element, index) => {
      if (index < current) {
        progress += `${completed} ${element}\n`;
      } else if (index === current) {
        progress += `${current_icon} ${element} - 현재 진행중\n`;
      } else {
        progress += `${pending} ${element}\n`;
      }
    });
  ...
  completionCriteria: [
    "브랜드만의 고유한 미션이 명확히 정의됨",
    "트리거 스토리와 연결된 비전이 구체적으로 설정됨",
    "3-5개의 핵심가치가 브랜드 특성에 맞게 정립됨",
    "구체적인 타깃 오디언스 프로필이 완성됨"
  ],

  helpPatterns: {
    keywords: ["모르겠", "어려워", "잘 모르", "글쎄", "패스", "다음", "그냥", "아무거나", "네가 정해", "추천해", "예시", "도와"],
    helpResponse: "걱정 마세요! Step 0에서 나누어주신 브랜드 이야기를 바탕으로 제가 구체적인 제안을 드릴게요."
  }
};
